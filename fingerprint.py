...
#load image to check
#sample = cv2.imread("101_1 (2).tif")
#sample = cv2.imread("103_3 (2)_corrupted.tif", 0)
sample = cv2.imread("103_3 (3).tif", 0)
...
for file in [file for file in os.listdir("fingers_data_base1")][:sizeOfDataSet]:
    if (counter/sizeOfDataSet * 100) % 10 == 0:
        print(counter/sizeOfDataSet * 100)
    counter += 1
    
    fingerTest = cv2.imread("fingers_data_base1/" + file, 0)
...
scoreToUnlock = 60.0
print("Required score to unlock: " + str(scoreToUnlock))
